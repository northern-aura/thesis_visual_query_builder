...
class MapDecodeStream(MapFunction):
    ...
    def map(self, encoded_data):
        ...
        return {
            ...
            "annotation_actions": decoded_data.get("gt_actions", []),
            "annotation_action_counts": decoded_data.get("gt_action_counts", {}),
            "annotation_visible_actions": decoded_data.get("gt_visible_actions", []),
            "annotation_visible_action_counts": decoded_data.get("gt_visible_action_counts", {}),
            "annotation_has_spiking": decoded_data.get("gt_has_spiking", False),
            "annotation_bboxes_spiking": decoded_data.get("gt_bboxes_spiking", []),
            "annotation_players": decoded_data.get("gt_players", []),
            "game_id": decoded_data.get("game_id", ""),
            "clip_id": decoded_data.get("clip_id", ""),
            "frame_id": decoded_data.get("frame_id", ""),
...
class MapResizeImage(MapFunction):
    ...
    def map(self, data):
        ...
        resized = cv.resize(image, (self.width, self.height), interpolation=cv.INTER_AREA)
        op_end = time.time()
...
class MapRecolorImage(MapFunction):
    ...
    def map(self, data):
        ...
        return {
            ...
            "sent" : data["sent"],
            "in" : data["in"],
            "latency_trace": data.get("latency_trace", []) + [{"operator": "Grayscale", "start": op_start, "end": op_end, "duration_ms": round((op_end - op_start) * 1000, 2)}]
        }
`;
}
...
export function generateMapDetectOrderedPairMethod() {
    return `

class MapDetectOrderedPair(MapFunction):
    def __init__(self, first_label, second_label, sequence_label=None):
        self.first = str(first_label).strip().upper()
        self.second = str(second_label).strip().upper()
        self.sequence_label = str(sequence_label).strip().upper() if sequence_label else f"{self.first}->{self.second}"
    ...
    def map(self, data):
        ...
        labels = [str(x).strip().upper() for x in data.get("result", [])]
        pending_first = 0
        ordered_pairs = 0
        for lbl in labels:
            if lbl == self.first:
                pending_first += 1
            elif lbl == self.second and pending_first > 0:
                ordered_pairs += 1
                pending_first -= 1
        ...
        llm_times = data.get("llm_time", [])
        avg_llm_time_ms = round(sum(llm_times) / len(llm_times) * 1000, 2) if llm_times else incoming_metrics.get("avg_llm_time_ms", 0)
        window_e2e_ms = round((data["in_last"] - data["sent_first"]) * 1000, 2) if "in_last" in data and "sent_first" in data else incoming_metrics.get("window_e2e_ms", 0)
        ...
        ann_actions = data.get("annotation_actions", [])
        evaluated = [i for i in range(len(results_raw)) if not (str(results_raw[i]).upper().strip() == "SKIP" and i < len(llm_times) and llm_times[i] == 0)]
        pred_norm = {i: normalize_action(results_raw[i]) for i in evaluated}
        gt_norm = {i: normalize_action(ann_actions[i]) for i in evaluated if i < len(ann_actions)}
        gt_labeled_idx = [i for i in evaluated if gt_norm.get(i, "")]
        if gt_labeled_idx:
            action_matches = sum(1 for i in gt_labeled_idx if pred_norm.get(i, "") == gt_norm.get(i, ""))
        ...
        return {
            "result": [(self.sequence_label, int(ordered_pairs))] if ordered_pairs > 0 else [],
            "sent_first": data["sent_first"],
            "sent_last": data["sent_last"],
            "in_first": data["in_first"],
            "in_last": data["in_last"],
            "llm_time": data.get("llm_time", []),
            "llm": data.get("llm", ""),
            "frames": frame_count,
            "sequence": {
...
export function generateMapCVColorFilterClass() {
    return `
...
export function generateMapFrameBatcherClass() {
    return `
class MapFrameBatcher(MapFunction):
    ...
    def map(self, data):
        ...
        # Not enough frames yet -> emit SKIP so downstream nodes pass through
        if len(self._buffer) < self.batch_size:
            ...
            return {
                ...
                "llm_time": 0,
                "latency_trace": data.get("latency_trace", []) + [
                    {"operator": "FrameBatcher", "start": op_start, "end": op_end,
                     "duration_ms": round((op_end - op_start) * 1000, 2)}
                ]
            }

        # We have batch_size frames -> emit the batch
        ...
        # Merge latency traces from all frames in the batch
        merged_traces = []
        for frame in batch:
            merged_traces.extend(frame.get("latency_trace", []))
        ...
        return {
            ...
            "annotation_plate": batch[0].get("annotation_plate", ""),
            "annotation_brand": batch[0].get("annotation_brand", ""),
            "annotation_color": batch[0].get("annotation_color", ""),
            "annotation_action": batch[0].get("annotation_action", ""),
            "annotation_actions": [frame.get("annotation_action", "") for frame in batch],
            "sent": batch[0]["sent"],
            "in": batch[0]["in"],
            "latency_trace": merged_traces
        }

`;
}

export function generateMapSkipFramesClass() {
    return `
...
export function generateMapComputeMetricsClass() {
    return `
