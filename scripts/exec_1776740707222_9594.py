...
kafka_source = KafkaSource.builder()\
  ...
  .set_group_id("dashboard_1776740700687_0")\
...
stream_4 = stream_3.map(MapPromptLLM("""Is there a license plate visible in this image? If yes, read the license plate text. If the license plate reads "QRF213" (ignoring whitespace), return "QRF213". Otherwise, return the string SKIP.
...
stream_metrics = stream_4.map(MapComputeMetrics("cars_specific_plate_grayscale"))

kafka_sink = KafkaSink.builder()\
  .set_bootstrap_servers("kafka:9093")\
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("cars_specific_plate_grayscale")\
...
env.execute("Specific Plate (Grayscale)")
