...
class MapSkipFrames(MapFunction):
    ...
    def map(self, data):
        op_start = time.time()
        # Pass skip config to downstream LLM via data dict
        data["_skip_amount"] = self.skip_count
        op_end = time.time()
...
class MapPromptLLM(MapFunction):
    ...
    def map(self, data):
        ...
        if "_skip_amount" in data:
            self._skip_amount = data.pop("_skip_amount")
        ...
        # If frame is empty (SKIP) and skip_frames is active, skip next M frames
        if answer.upper().strip() == "SKIP" and self._skip_amount > 0:
            self._skip_countdown = self._skip_amount
...
stream_2 = stream_1.map(MapSkipFrames(15))
