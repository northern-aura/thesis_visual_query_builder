...
stream_3 = stream_2.map(MapPromptLLM("""Is there a car visible in this image? If yes, is the car red (do not take into consideration the bumper)? If the car is red, return "red". If the car is not red or there is no car, return the string SKIP.

Do not provide any text or any explanation, just the final answer in the asked format!""", """gpt-4o"""))
# Procesing Node 4 (filter)
stream_4 = stream_3.filter(FilterNotEmpty())
# Procesing Node 5 (filter)
stream_5 = stream_4.filter(FilterNotSkipped())

# Compute metrics (accuracy, latency) before sending to sink
stream_metrics = stream_5.map(MapComputeMetrics())
