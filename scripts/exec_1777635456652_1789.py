...
kafka_source = KafkaSource.builder()\
  ...
  .set_group_id("dashboard_1777635442288_0")\
...
stream_3 = stream_2.map(MapPromptLLM("""Look at this volleyball image. Is the dominant action setting, blocking, or neither? Reply with one of: set, block, other. If no players visible, return SKIP.
...
stream_5 = stream_4.map(MapDetectOrderedPair("set", "block", "set->block"))

# Compute metrics (accuracy, latency) before sending to sink
stream_metrics = stream_5.map(MapComputeMetrics("opt_vb_set_block_r854"))

kafka_sink = KafkaSink.builder()\
  .set_bootstrap_servers("kafka:9093")\
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("opt_vb_set_block_r854")\
...
env.execute("Optimized Set->Block Events (R854)")
