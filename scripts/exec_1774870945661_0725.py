...
# Procesing Node 6 (window)
stream_6 = stream_5.map(MapChangeFormat()).window_all(TumblingEventTimeWindows.of(Time.seconds(10))).reduce(ReduceWindowResults())
# Procesing Node 7 (aggr)
stream_7 = stream_6

# Compute metrics (accuracy, latency) before sending to sink
stream_metrics = stream_7.map(MapComputeMetrics())
