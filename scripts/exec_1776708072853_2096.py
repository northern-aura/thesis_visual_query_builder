...
kafka_source = KafkaSource.builder()\
  ...
  .set_group_id("dashboard_1776708052733_0")\
...
stream_2 = stream_1.map(MapPromptLLM("""Is there a car visible in this image? If yes, is the car red (do not take into consideration the bumper)? If the car is red, return the brand of the car in lowercase (e.g. ford, renault, toyota, bmw). If the car is not red or there is no car, return the string SKIP.
...
stream_metrics = stream_4.map(MapComputeMetrics("cars_most_popular_brand_red"))

kafka_sink = KafkaSink.builder()\
  .set_bootstrap_servers("kafka:9093")\
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("cars_most_popular_brand_red")\
...
env.execute("Most Popular Brand (Red Cars)")
