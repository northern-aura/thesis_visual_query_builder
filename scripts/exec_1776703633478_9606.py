...
kafka_source = KafkaSource.builder()\
  ...
  .set_group_id("dashboard_1776703626931_0")\
...
stream_3 = stream_2.map(MapResizeImage(640,360))
# Procesing Node 4 (llm)
stream_4 = stream_3.map(MapPromptLLM("""Is there a license plate visible in this image? If yes, return the text of the license plate in uppercase without any whitespaces. If there is no license plate in the image, return the string SKIP.
...
stream_metrics = stream_7.map(MapComputeMetrics("cars_repeating_plates_grayscale"))

kafka_sink = KafkaSink.builder()\
  .set_bootstrap_servers("kafka:9093")\
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("cars_repeating_plates_grayscale")\
...
env.execute("Repeating Plates (Grayscale)")
