...
kafka_source = KafkaSource.builder()\
  ...
  .set_group_id("dashboard_1777418706219_0")\
...
stream_2 = stream_1.map(MapCVColorFilter("cv_color_grey", 2))
# Procesing Node 3 (resize)
stream_3 = stream_2.map(MapResizeImage(480,270))
# Procesing Node 4 (llm)
stream_4 = stream_3.map(MapPromptLLM("""Is there a car visible in this image? If yes, is the car gray or silver (do not take into consideration the bumper)? If the car is gray or silver, return "gray". If the car is not gray/silver or there is no car, return the string SKIP.
...
stream_metrics = stream_4.map(MapComputeMetrics("opt_cars_color_grey_cfgrey_r480"))

kafka_sink = KafkaSink.builder()\
  .set_bootstrap_servers("kafka:9093")\
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("opt_cars_color_grey_cfgrey_r480")\
...
env.execute("Optimized Car Color Grey (CFgrey+R480)")
