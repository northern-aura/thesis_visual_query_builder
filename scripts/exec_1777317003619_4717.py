...
kafka_source = KafkaSource.builder()\
  ...
  .set_group_id("dashboard_1777316981694_0")\
...
stream_3 = stream_2.map(MapPromptLLM("""Is there a player spiking the ball in this volleyball image? If yes, return 'spiking'. If not or no players visible, return SKIP.
...
stream_5 = stream_4.map(MapGetCounts()).map(MapRemoveLowCount(3))
# Procesing Node 6 (aggr)
stream_6 = stream_5.map(MapGetCounts())

# Compute metrics (accuracy, latency) before sending to sink
stream_metrics = stream_6.map(MapComputeMetrics("opt_vb_repeated_spikers_r854"))

kafka_sink = KafkaSink.builder()\
  .set_bootstrap_servers("kafka:9093")\
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("opt_vb_repeated_spikers_r854")\
...
env.execute("Optimized Repeated Spikers (R854)")
