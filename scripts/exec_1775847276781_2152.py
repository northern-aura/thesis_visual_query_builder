...
class MapComputeMetrics(MapFunction):
    ...
    def map(self, data):
        ...
        # Accuracy (compare result to ground truth — skip frames are excluded)
        result_val = data.get("result", "").strip()
        accuracy = {}
        if result_val.upper() != "SKIP":
    ...
    def close(self):
        ...
        if is_windowed:
            ...
            results_list = [r.get("result", "") for r in self._results]
            skipped = sum(1 for r in results_list if r.upper().strip() == "SKIP")
            processed = total - skipped

            llm_times = [r.get("metrics", {}).get("llm_time_ms", 0) for r in self._results if r.get("result", "").upper().strip() != "SKIP"]
...
stream_metrics = stream_2.map(MapComputeMetrics("cars_query_car_color_recognition_optimised_skipping"))
