...
kafka_source = KafkaSource.builder()\
  ...
  .set_group_id("dashboard_1777654050304_0")\
...
stream_2 = stream_1.map(MapResizeImage(854,480))
# Procesing Node 3 (llm)
stream_3 = stream_2.map(MapPromptLLM("""Look at this volleyball image. Is the dominant action jumping, spiking, or neither? Reply with one of: jump, spike, other. If no players visible, return SKIP.

Do not provide any text or any explanation.""", """qwen2.5-vl-3b"""))
# Procesing Node 4 (window)
stream_4 = stream_3.map(MapChangeFormat()).key_by(lambda _: 0, key_type=Types.INT()).count_window(20).reduce(ReduceWindowResults())
# Procesing Node 5 (aggr)
stream_5 = stream_4.map(MapDetectOrderedPair("jump", "spike", "jump->spike"))

# Compute metrics (accuracy, latency) before sending to sink
stream_metrics = stream_5.map(MapComputeMetrics("opt_vb_jump_spike_r854"))

kafka_sink = KafkaSink.builder()\
  .set_bootstrap_servers("kafka:9093")\
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("opt_vb_jump_spike_r854")\
...
env.execute("Optimized Jump->Spike Events (R854)")
