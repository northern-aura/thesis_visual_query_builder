...
kafka_source = KafkaSource.builder()\
  ...
  .set_group_id("dashboard_1777628644875_0")\
...
stream_2 = stream_1.map(MapPromptLLM("""Look at this volleyball image. Is the dominant action jumping, spiking, or neither? Reply with one of: jump, spike, other. If no players visible, return SKIP.
...
stream_4 = stream_3.map(MapDetectOrderedPair("jump", "spike", "jump->spike"))

# Compute metrics (accuracy, latency) before sending to sink
stream_metrics = stream_4.map(MapComputeMetrics("volleyball_jump_spike"))

kafka_sink = KafkaSink.builder()\
  .set_bootstrap_servers("kafka:9093")\
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("volleyball_jump_spike")\
...
env.execute("Jump→Spike Events (Window)")
