...
from llm_call import prompt_llm, send_to_gpt, send_to_ollama, send_to_vllm
...
class MapPromptLLM(MapFunction):
    ...
    def map(self, data):
        ...
        if self.model == "gpt-4o":
            ...
            answer, llm = send_to_vllm("RedHatAI/Qwen2.5-VL-7B-Instruct-quantized.w8a8", data["image"], self.prompt)
        else:
...
stream_2 = stream_1.map(MapPromptLLM("""Is there a car visible in this image? If yes, is the car gray or silver (do not take into consideration the bumper)? If the car is gray or silver, return "gray". If the car is not gray/silver or there is no car, return the string SKIP.
...
stream_metrics = stream_2.map(MapComputeMetrics("cars_color_grey"))

kafka_sink = KafkaSink.builder()\
  .set_bootstrap_servers("kafka:9093")\
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("cars_color_grey")\
