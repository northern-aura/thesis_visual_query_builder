...
kafka_source = KafkaSource.builder()\
  ...
  .set_group_id("dashboard_1777647570433_0")\
...
stream_2 = stream_1.map(MapResizeImage(960,540))
# Procesing Node 3 (frame batcher)
stream_3 = stream_2.map(MapFrameBatcher(3))
# Procesing Node 4 (llm)
stream_4 = stream_3.map(MapPromptLLM("""Is a volleyball block happening at the net in this image? If yes, return the bounding box of the blocking player as 'x1,y1,x2,y2' (pixel integers). If no block, return SKIP.

Do not provide any text or any explanation.""", """qwen2.5-vl-3b"""))
# Procesing Node 5 (window)
stream_5 = stream_4.map(MapChangeFormat()).key_by(lambda _: 0, key_type=Types.INT()).count_window(20).reduce(ReduceWindowResults())
# Procesing Node 6 (aggr)
stream_6 = stream_5.map(MapGetCounts())

# Compute metrics (accuracy, latency) before sending to sink
stream_metrics = stream_6.map(MapComputeMetrics("opt_vb_bbox_block_r960_b3_w20"))

kafka_sink = KafkaSink.builder()\
  .set_bootstrap_servers("kafka:9093")\
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("opt_vb_bbox_block_r960_b3_w20")\
...
env.execute("Optimized Block Bounding Boxes (R960+B3+W20)")
