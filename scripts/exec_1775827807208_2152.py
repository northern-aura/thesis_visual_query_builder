...
class MapSkipFrames(MapFunction):
    def __init__(self, skip_count):
        ...
        SKIP_AMOUNT = self.skip_count  # Store for MapPromptLLM to reference

    def map(self, data):
        global SKIP_COUNTDOWN
        global SKIP_AMOUNT
        SKIP_AMOUNT = self.skip_count  # Must set here — __init__ runs on driver, map() runs on worker
        ...
        if SKIP_COUNTDOWN > 0:
            SKIP_COUNTDOWN -= 1
            op_end = time.time()
            # Mark as SKIP — downstream LLM will short-circuit (no API call)
            data["result"] = "SKIP"
        ...
        # Not skipping — pass frame through for LLM processing
        op_end = time.time()
        data["latency_trace"] = data.get("latency_trace", []) + [
            {"operator": "SkipFrames", "start": op_start, "end": op_end,
             "duration_ms": round((op_end - op_start) * 1000, 2)}
...
class MapPromptLLM(MapFunction):
    ...
    def map(self, data):
        ...
        # If frame is empty (SKIP) and skip_frames node is active, trigger skip for next M frames
        if answer.upper().strip() == "SKIP" and SKIP_AMOUNT > 0:
...
stream_2 = stream_1.map(MapSkipFrames(7))
...
stream_metrics = stream_3.map(MapComputeMetrics())

kafka_sink = KafkaSink.builder()\
  .set_bootstrap_servers("kafka:9093")\
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("cars_query_car_color_recognition_optimised_skipping")\
...
env.execute("Query Car Color Recognition")
