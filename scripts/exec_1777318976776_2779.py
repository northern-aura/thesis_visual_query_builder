...
kafka_source = KafkaSource.builder()\
  ...
  .set_group_id("dashboard_1777318958446_0")\
...
stream_2 = stream_1.map(MapPromptLLM("""Look at this volleyball image. What is the dominant player action? Reply with exactly one of: waiting, setting, digging, falling, jumping, moving, blocking, standing, spiking. If no players are visible, return SKIP.
...
stream_metrics = stream_4.map(MapComputeMetrics("volleyball_top_actions"))

kafka_sink = KafkaSink.builder()\
  .set_bootstrap_servers("kafka:9093")\
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("volleyball_top_actions")\
...
env.execute("Top Actions (Window)")
