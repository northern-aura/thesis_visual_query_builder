...
kafka_source = KafkaSource.builder()\
  ...
  .set_group_id("dashboard_1777443757129_0")\
...
# Procesing Node 2 (cv color filter)
stream_2 = stream_1.map(MapCVColorFilter("cv_color_red", 2))
# Procesing Node 3 (resize)
stream_3 = stream_2.map(MapResizeImage(854,480))
# Procesing Node 4 (llm)
stream_4 = stream_3.map(MapPromptLLM("""Is there a car visible in this image? If yes, is the car red (do not take into consideration the bumper)? If the car is red, return the brand of the car in lowercase (e.g. ford, renault, toyota, bmw). If the car is not red or there is no car, return the string SKIP.
...
stream_5 = stream_4.map(MapChangeFormat()).window_all(TumblingProcessingTimeWindows.of(Time.seconds(30))).reduce(ReduceWindowResults())
# Procesing Node 6 (aggr)
stream_6 = stream_5.map(MapGetCounts())

# Compute metrics (accuracy, latency) before sending to sink
stream_metrics = stream_6.map(MapComputeMetrics("opt_cars_most_popular_brand_red_cfred_r854"))

kafka_sink = KafkaSink.builder()\
  .set_bootstrap_servers("kafka:9093")\
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("opt_cars_most_popular_brand_red_cfred_r854")\
...
env.execute("Optimized Most Popular Brand Red (CFred+R854)")
