...
kafka_source = KafkaSource.builder()\
  ...
  .set_group_id("dashboard_1776635877682_0")\
...
stream_2 = stream_1.map(MapPromptLLM("""Is there a car visible in this image? If yes, is the car white (do not take into consideration the bumper)? If the car is white, return "white". If the car is not white or there is no car, return the string SKIP.
...
stream_metrics = stream_2.map(MapComputeMetrics("cars_color_white"))

kafka_sink = KafkaSink.builder()\
  .set_bootstrap_servers("kafka:9093")\
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("cars_color_white")\
