...
kafka_source = KafkaSource.builder()\
  ...
  .set_group_id("dashboard_1776608758225_0")\
...
stream_2 = stream_1.map(MapPromptLLM("""Is there a car visible in this image? If yes, is it a Ford? If it is a Ford, return "ford". If the car is not a Ford or there is no car, return the string SKIP.
...
stream_metrics = stream_2.map(MapComputeMetrics("cars_brand_ford"))

kafka_sink = KafkaSink.builder()\
  .set_bootstrap_servers("kafka:9093")\
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("cars_brand_ford")\
