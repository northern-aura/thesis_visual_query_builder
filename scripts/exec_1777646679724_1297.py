...
kafka_source = KafkaSource.builder()\
  ...
  .set_group_id("dashboard_1777646653001_0")\
...
# Procesing Node 2 (grayscale)
stream_2 = stream_1.map(MapRecolorImage())
# Procesing Node 3 (resize)
stream_3 = stream_2.map(MapResizeImage(854,480))
# Procesing Node 4 (llm)
stream_4 = stream_3.map(MapPromptLLM("""Is there a player blocking at the net in this volleyball image? If yes, return 'blocking'. If not or no players visible, return SKIP.
...
stream_6 = stream_5.map(MapGetCounts()).map(MapRemoveLowCount(3))
# Procesing Node 7 (aggr)
stream_7 = stream_6.map(MapGetCounts())

# Compute metrics (accuracy, latency) before sending to sink
stream_metrics = stream_7.map(MapComputeMetrics("opt_vb_repeated_blockers_g_r854"))

kafka_sink = KafkaSink.builder()\
  .set_bootstrap_servers("kafka:9093")\
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("opt_vb_repeated_blockers_g_r854")\
...
env.execute("Optimized Repeated Blockers (G+R854)")
