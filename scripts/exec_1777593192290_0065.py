...
kafka_source = KafkaSource.builder()\
  ...
  .set_group_id("dashboard_1777593170630_0")\
...
stream_2 = stream_1.map(MapPromptLLM("""Look at this volleyball image. Roughly what fraction of visible players are moving (running/jumping/diving) vs. standing still? Reply with exactly one of: ALL_MOVING, MOST_MOVING, SOME_MOVING, NONE_MOVING. If no players visible, return SKIP.
...
stream_metrics = stream_4.map(MapComputeMetrics("volleyball_motion_category"))

kafka_sink = KafkaSink.builder()\
  .set_bootstrap_servers("kafka:9093")\
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("volleyball_motion_category")\
...
env.execute("Motion Category (Window)")
