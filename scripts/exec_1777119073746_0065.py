...
class MapGetCounts(MapFunction):
    def map(self, data):
        ...
        if not accuracy and isinstance(incoming_metrics.get("accuracy"), dict):
            accuracy = incoming_metrics.get("accuracy", {})
...
kafka_source = KafkaSource.builder()\
  ...
  .set_group_id("dashboard_1777119066612_0")\
...
stream_3 = stream_2.map(MapChangeFormat()).window_all(TumblingProcessingTimeWindows.of(Time.seconds(120))).reduce(ReduceWindowResults())
