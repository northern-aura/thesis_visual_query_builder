...
kafka_source = KafkaSource.builder()\
  ...
  .set_group_id("dashboard_1777595739291_0")\
...
stream_2 = stream_1.map(MapPromptLLM("""Are there players moving (running) in this volleyball image? Return 'moving' if at least one player is moving, otherwise SKIP.
...
stream_metrics = stream_4.map(MapComputeMetrics("volleyball_players_moving"))

kafka_sink = KafkaSink.builder()\
  .set_bootstrap_servers("kafka:9093")\
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("volleyball_players_moving")\
...
env.execute("Players Moving (Window)")
