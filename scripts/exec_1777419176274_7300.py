...
kafka_source = KafkaSource.builder()\
  ...
  .set_group_id("dashboard_1777419163459_0")\
...
stream_4 = stream_3.map(MapChangeFormat()).window_all(TumblingProcessingTimeWindows.of(Time.seconds(30))).reduce(ReduceWindowResults())
# Procesing Node 5 (aggr)
stream_5 = stream_4.map(MapGetCounts())

# Compute metrics (accuracy, latency) before sending to sink
stream_metrics = stream_5.map(MapComputeMetrics("opt_cars_most_popular_color_ford_r854"))

kafka_sink = KafkaSink.builder()\
  .set_bootstrap_servers("kafka:9093")\
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("opt_cars_most_popular_color_ford_r854")\
...
env.execute("Optimized Most Popular Color Ford (R854)")
