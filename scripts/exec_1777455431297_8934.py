...
kafka_source = KafkaSource.builder()\
  ...
  .set_group_id("dashboard_1777455409272_0")\
...
stream_3 = stream_2.map(MapPromptLLM("""Look at this volleyball image. Is the dominant action setting, spiking, or neither? Reply with one of: set, spike, other. If no players visible, return SKIP.
...
stream_5 = stream_4.map(MapDetectOrderedPair("set", "spike", "set->spike"))

# Compute metrics (accuracy, latency) before sending to sink
stream_metrics = stream_5.map(MapComputeMetrics("opt_vb_set_spike_r854"))

kafka_sink = KafkaSink.builder()\
  .set_bootstrap_servers("kafka:9093")\
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("opt_vb_set_spike_r854")\
...
env.execute("Optimized Set->Spike Events (R854)")
