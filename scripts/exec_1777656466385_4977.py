...
kafka_source = KafkaSource.builder()\
  ...
  .set_group_id("dashboard_1777656447932_0")\
...
stream_3 = stream_2.map(MapPromptLLM("""Are there players moving (running) in this volleyball image? Return 'moving' if at least one player is moving, otherwise SKIP.
...
stream_metrics = stream_5.map(MapComputeMetrics("opt_vb_players_moving_r480"))

kafka_sink = KafkaSink.builder()\
  .set_bootstrap_servers("kafka:9093")\
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("opt_vb_players_moving_r480")\
...
env.execute("Optimized Players Moving (R480)")
