...
kafka_source = KafkaSource.builder()\
  ...
  .set_group_id("dashboard_1777315239491_0")\
...
stream_2 = stream_1.map(MapPromptLLM("""Look at this volleyball image. Is the dominant action setting, blocking, or neither? Reply with one of: set, block, other. If no players visible, return SKIP.
...
stream_4 = stream_3.map(MapDetectOrderedPair("set", "block", "set->block"))

# Compute metrics (accuracy, latency) before sending to sink
stream_metrics = stream_4.map(MapComputeMetrics("volleyball_set_block"))

kafka_sink = KafkaSink.builder()\
  .set_bootstrap_servers("kafka:9093")\
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("volleyball_set_block")\
...
env.execute("Set→Block Events (Window)")
