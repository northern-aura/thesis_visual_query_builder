...
# Procesing Node 3 (filter)
stream_3 = stream_2.filter(FilterNotEmpty())
# Procesing Node 4 (filter)
stream_4 = stream_3.filter(FilterNotSkipped())
# Procesing Node 5 (window)
stream_5 = stream_4.map(MapChangeFormat()).window_all(TumblingProcessingTimeWindows.of(Time.seconds(10))).reduce(ReduceWindowResults())
...
kafka_sink = KafkaSink.builder()\
  ...
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("cars_most_popular_brand_and_color")\
...
env.execute("Most Popular Brand and Color")
