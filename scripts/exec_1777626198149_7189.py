...
kafka_source = KafkaSource.builder()\
  ...
  .set_group_id("dashboard_1777626176536_0")\
...
stream_2 = stream_1.map(MapPromptLLM("""Look at this volleyball image. Is the dominant action digging, setting, or neither? Reply with one of: dig, set, other. If no players visible, return SKIP.
...
stream_4 = stream_3.map(MapDetectOrderedPair("dig", "set", "dig->set"))

# Compute metrics (accuracy, latency) before sending to sink
stream_metrics = stream_4.map(MapComputeMetrics("volleyball_dig_set"))

kafka_sink = KafkaSink.builder()\
  .set_bootstrap_servers("kafka:9093")\
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("volleyball_dig_set")\
...
env.execute("Dig→Set Events (Window)")
