...
kafka_source = KafkaSource.builder()\
  ...
  .set_group_id("dashboard_1777452500170_0")\
...
stream_3 = stream_2.map(MapPromptLLM("""Look at this volleyball image. What is the dominant player action? Reply with exactly one of: waiting, setting, digging, falling, jumping, moving, blocking, standing, spiking. If no players are visible, return SKIP.
...
stream_metrics = stream_5.map(MapComputeMetrics("opt_vb_top_actions_r854"))

kafka_sink = KafkaSink.builder()\
  .set_bootstrap_servers("kafka:9093")\
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("opt_vb_top_actions_r854")\
...
env.execute("Optimized Top Actions (R854)")
