...
kafka_source = KafkaSource.builder()\
  ...
  .set_group_id("dashboard_1776618135489_0")\
...
stream_2 = stream_1.map(MapPromptLLM("""Is there a car visible in this image? If yes, is it a Toyota? If it is a Toyota, return "toyota". If the car is not a Toyota or there is no car, return the string SKIP.
...
stream_metrics = stream_2.map(MapComputeMetrics("cars_brand_toyota"))

kafka_sink = KafkaSink.builder()\
  .set_bootstrap_servers("kafka:9093")\
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("cars_brand_toyota")\
