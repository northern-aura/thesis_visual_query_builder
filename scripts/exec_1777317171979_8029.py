...
kafka_source = KafkaSource.builder()\
  ...
  .set_group_id("dashboard_1777317149059_0")\
...
stream_2 = stream_1.map(MapResizeImage(480,270))
# Procesing Node 3 (llm)
stream_3 = stream_2.map(MapPromptLLM("""Look at this volleyball image. Roughly what fraction of visible players are moving (running/jumping/diving) vs. standing still? Reply with exactly one of: ALL_MOVING, MOST_MOVING, SOME_MOVING, NONE_MOVING. If no players visible, return SKIP.
...
stream_metrics = stream_5.map(MapComputeMetrics("opt_vb_motion_category_r480"))

kafka_sink = KafkaSink.builder()\
  .set_bootstrap_servers("kafka:9093")\
  .set_record_serializer(KafkaRecordSerializationSchema.builder()\
       .set_topic("opt_vb_motion_category_r480")\
...
env.execute("Optimized Motion Category (R480)")
